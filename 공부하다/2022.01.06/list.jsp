...
<script type="text/javascript">
	...
	//장바구니 담기 
	$("#table-books .btn-outline-primary").click(function(){
		//var $button = $(this);
		var bookNo = $(this).attr('data-book-no');
		
		$.getJSON("/rest/cart/add.do", {bookNo:bookNo}, function(response){ //보낼경로, 보낼 변수이름,설정변수이름, 내려받는 이름 
			/*
				response = {status : "OK", error:null, items:[5]}
				장바구니 담기가 성공했을 때 응답 데이터
			*/
			if(response.status == "OK") {
				...
				/*
					장바구니 담기가 실해했을 때 응답 데이터
					response = {status : "FAIL", error:"동일한 상품이 이미 저장되어 있습니다. ", items:null}
				*/
