...
public class App {
	...
	private void searchFoodList() throws SQLException {
		...
		for(Food food : foods) {
			...
			System.out.println(food.getNo() + "\t" + food.getCategory() + "\t" +food.getName() + "\t" +food.getPrice() + "\t" +food.getOnSale());
			System.out.println("----------------------------------------------------------------------------------------------------");
		}
	...
	private void searchFoodInfo() throws SQLException {
		...
		System.out.println("카테고리 이름: " + food.getCategory());
	...
	private void order() throws SQLException {
		...
		for(int i = 0; i <= findtexts.length; i += 2 ) {
			...
			buyAmount = findtexts[i+1];
			foodNumber = Integer.parseInt(foodText);
			amount = Integer.parseInt(buyAmount);
			putOrder.setOrderNo(foodNumber);
			putOrder.setAmount(amount);
