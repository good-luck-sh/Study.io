...
<div class="container">    
	...
	<div class="row mb-3">
		<div class="col">
			<table class="table">
				...
				<%--
					요청객체에 "todoDtoList"속성명으로 저장된 객체를 조회하면 List<TodoDto>객체가 획득된다. 
					<c:forEach />태그를 이용해서 조회된 List<TodoDto>객체에 저장된 TodoDto객체의 갯수만큼 tr태그를 출력한다. 
				 --%>
				<tbody>
				<%--
	...
	<div class="row mb-3">
	<%--
		요청객체에 "pagin"속성명으로 저장된 객체를 조회하면 Pagination객체가 조회된다.
		Pagination객체에는 
			isExistPrev(),isExistNext(), getPrev(), getNext(), getBeginPage(), getEndPage(), getPage() 메소드를 제공한다.
			이전블록존재여부, 다음블록존재여부, 이전페이지번호, 다음페이지번호, 시작페이지번호, 끝페이지번호, 현재페이지 번호를 반환한다.
	 --%>
		<div class="col">
		<%--
