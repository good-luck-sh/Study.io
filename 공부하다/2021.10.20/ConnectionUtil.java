...
/**
 * JDBC 드라이버를 등록하고, RDBMS와 연결된 Connection객체를 제공하는 유틸리티 클래스다. 
 * @author lion3
 *
 */
public class ConnectionUtil {
	
	//문자열은 잘 못 적어도 에러가 안나기 때문에 신경써서 입력을 해야한다. 
	...
	//정적 초기화 블록
	//클래스가 메모리에 로딩될 떄 즉시 실행된다. 
	//객체 생성과 상관없이 클래스가 메모리에 로딩되는 즉시 실행된다. 
	//클레스가 메모리에 로딩되는 시점은 수행문에서 클래스 이름이 처음 등장하는 순간이다. 
	//정적 초기화 블록은 클래스의 일생동안 단 한번만 실행된다. 
	//JDBC 드라이버를 JVM의 드라이버 레지스트리에 등록하는 작업도 딱 한번도 등록되면 되기 때문에 
	//정적 초기화 블록으로 작성하였다. 
	 ...
	 * @return RDBMS 와 연결을 담당하는 Connection 객체
