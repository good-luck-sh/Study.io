...
public class RunningTimeCheckAdvice {
	...
	@Autowired
	/*
	 * 대상 메소드 실행 전후에 실행할 공통기능을 구현하는 Advice메소드
	 * 		@Around
	 * 			+ 반환타입은 반드시 Object로 정한다. 
	 * 			+ 메소드 이름은 상관없다.
	 * 			+ 매개변수에는 반드시 ProceedingJoinPoint 타입의 매개변수를 포함해야한다.
	 * 			+ 예외는 Throwable를 던진다.
	 * 		+ 대상 메소드 실행 전,후에 실행할 공통기능임을 나타낸다.
	 * 	execution(* com.sample.service.*Service.*(..))(반환타입은 아무거나 상관없다 클래명 (몇개던지 상관없다.))
	 * 		+ AspectJ 표현식이다.
	 * 		+ 공통기능이 적용될 곳을 지정한다.
	 * 		+ within(패키지를 포함한 클래스명)
	 * 			예) within(com.sample.service.BookService)
	 ...
	 * 				+ 클래스 이름에 *를 붙이면 any라는 의미이다. *Service는 BookService, UserService, CartItemService를 모두 포함한다.
	 * 		+ execution(반환타입 패키지명을 포함한 클래스와 메소드명)
	 * 			예) execution(* com.sample.dao.BookDao.get*(..))
	 * 					-com.sample.dao패키지의 BookDao클래스에서 get으로 시작하는 모든 메소드가 실행될 때 
	 * 			   execution(* com.sample.dao.BookDao.update*(*))
	 * 					-com.sample.dao패키지의 BookDao클래스애서 update로 시작하고, 매개변수 한 개 전달받는 메소드가 실행될 떄 
	...
	public Object runningTimeCheck(ProceedingJoinPoint jointPoint) throws Throwable {//생략할수없다
		...
		returnValue = jointPoint.proceed();		//@Around Advice가 적용되는 대상메소드를 실행시키는 코드
		//값이 없으면 null값이 들어간다. 
		...
		return returnValue;
			...
			System.out.println("총 소요시간" + stopWatch.getTotalTimeMillis() + "밀리초 소요됨");
		}
